...
class CascadeForm(forms.Form):
    ...
    def __init__(self, ad_id, phase_id, activity_id, task_id, project_id, cycle_id, *args, **kwargs):
        ...
        for k, v in get_cascade_phase_activity_task_by_their_id(phase_id, activity_id, task_id, project_id, cycle_id).items():
            if k == "activities":
        ...
        [query_result_phases.append((o.id, o.name)) for o in Phase.objects.get_objects_by_general_filtre(request=None, attrs={'project_id': project_id, 'cycle_id': cycle_id}).order_by("order")] 
        ...
        if task_id:
            ...
            _a = Activity.objects.filter(id=int(activity_id)).get_objects_by_general_filtre(request=None, attrs={'project_id': project_id, 'cycle_id': cycle_id})
            if _a:
