...
class FilterFacilitatorForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['region'].widget.choices = [('', '')] + [(str(o.id), o.name) for o in AdministrativeLevel.objects.using('mis').filter(type="Region").order_by("name")]
        self.fields['prefecture'].widget.choices = [('', '')] + [(str(o.id), o.name) for o in AdministrativeLevel.objects.using('mis').filter(type="Prefecture").order_by("name")]
        self.fields['commune'].widget.choices = [('', '')] + [(str(o.id), o.name) for o in AdministrativeLevel.objects.using('mis').filter(type="Commune").order_by("name")]
        self.fields['canton'].widget.choices = [('', '')] + [(str(o.id), o.name) for o in AdministrativeLevel.objects.using('mis').filter(type="Canton").order_by("name")]
