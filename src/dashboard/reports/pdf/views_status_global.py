...
class Generate(View):
    def generate(self, request, facilitator_db_name=None):
        ...
        projects = Project.objects.get(id=self.request.session.get('project_id')).build_the_tree_structure()
...
class GeneratePDF(Generate):
    def get(self, request, facilitator_db_name=None, *args, **kwargs):
        ...
        template_page = "reports/pdf/facilitators_status_global.html"
        ...
        if pdf:
            ...
            nom = '{}'.format('report_facilitators_global' if id_in_details == "0" else (context['facilitators'][0].get("facilitator").get('name') if len(context['facilitators']) == 1 else 'report_facilitator'))
